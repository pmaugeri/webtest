...
class AkamaiPragmaHeaders {
  ...
  /**
   * This method will return a Pragma HTTP header value. Note that it could be used to retrieve any HTTP header from the request.
   *
   * @param String the HTTP header name to search
   * @return String return the HTTP header, or undefined if not found
   */
  ...
  /**
   * @return String the X-Cache header, or undefined if the HTTP header could not be found
   */
  getXCacheHeader() {
  ...
  /**
   * @return String return the TTL value found in X-Cache-Key header
   */
  getTTL() {
    var xCacheKey = this.getXCacheKey();
    if (xCacheKey != undefined)
